...
fn solve(content: &String) -> i32 {
    ...
    //     println!("{k} {v:?}");
    // }
    let mut n_simulations = 0;
    loop {
        n_simulations += 1;
        if n_simulations % 100000 == 0 {
            println!("n_sim {n_simulations}");
        }
        let rx_hit = rx_received_low_pulse(&mut modules, &transition);
        if rx_hit {
            break;
        }
    }
